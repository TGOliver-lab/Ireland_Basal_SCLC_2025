...
# Annotate by pattern here
library(ggplot2)

# Example: Assuming your data is stored in a data frame
clones_df <- clones@meta.data  # Extract metadata for plotting
clones_df
# Create a bar plot with annotations
library(ggbreak)
ggplot(clones_df, aes(x = factor(CellTag_Clone), fill = CellTag_Pattern)) +
  geom_bar() +
  theme_minimal() + scale_y_break(c(5, 1200)) +
  labs(x = "Clusters", y = "Cell Count", fill = "CellTag Pattern") +
...
#Order of clones (obtain from manual sort based on group patterns)
